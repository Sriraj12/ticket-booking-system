...
function BookingSuccessContent() {
    ...
    const { data: bookingData, error, isLoading } = useSWR(
        !isNaN(bookingId) ? `/user/bookings/${bookingId}` : null
    );
    ...
    // useEffect(() => {
    //     if(!booking && seatDetails.length === 0) return;
    //     const formattedSeats = formatSeats(seatDetails);
    //     setFormattedSeats(formattedSeats);
    // }, [seatDetails]);
    ...
    const formattedSeats = formatSeats(booking.seats);
            ...
            <div className="fixed inset-0 pointer-events-none overflow-hidden">
                {[...Array(12)].map((_, i) => (
                    <motion.div
                        ...
                        className={`absolute w-2 h-2 rounded-full ${i % 2 === 0
                                ? "bg-amber-400"
                                : i % 3 === 0
                                    ? "bg-red-500"
                                    : "bg-yellow-400"
            ...
            >
                ...
                >
                    ...
                    <motion.button
                        ...
                        whileHover={{ scale: 1.02 }}
                        whileTap={{ scale: 0.98 }}
                        className="flex-1 py-4 bg-black/5 border border-slate-600 text-black font-bold rounded-lg hover:bg-slate-200/50 transition-all flex items-center justify-center gap-2"
                    >
