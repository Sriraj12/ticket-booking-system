...
function SeatSelectionContent() {
    ...
    const getSeatColor = (seat) => {
        ...
        if (selectedSeats.includes(seat.id)) return "bg-gradient-to-r from-black to-slate-700";
    ...
    return (
        <motion.div
            ...
            animate={{ opacity: 1 }}
            transition={{ duration: 0.5 }}
            className="min-h-screen bg-gradient-to-br from-white via-slate-100 to-white py-12 px-6 flex flex-col"
        >
            {/* Header */}
            <Header />
            ...
            >
                <div className="flex items-center gap-3 mb-4">
                    ...
                    <h1 className="text-4xl font-bold bg-gradient-to-r from-black to-slate-700 bg-clip-text text-transparent">
                        Select Your Seats
                    </h1>
                </div>
                <p className="text-slate-600 text-lg">{movieTitle}</p>
            </motion.div>
            ...
            <motion.button
                whileHover={{ x: -5 }}
                onClick={() => router.back()}
                className="mb-6 flex items-center gap-2 text-black font-semibold hover:text-slate-900 transition-colors"
            >
                ← Back to Show Times
            </motion.button>
            ...
            <div className="flex-1 flex flex-col lg:flex-row gap-8 max-w-7xl mx-auto w-full">
                    ...
                    >
                        <div className="relative">
                            <div className="w-full h-16 bg-gradient-to-r from-slate-700 via-slate-600 to-slate-700 rounded-t-full shadow-2xl border border-slate-500/50 flex items-center justify-center">
                                <span className="text-slate-100 font-semibold text-lg">SCREEN</span>
                            </div>
                            <div className="absolute inset-0 bg-gradient-to-r from-black/10 to-slate-700/10 rounded-t-full blur-sm"></div>
                        </div>
                        <p className="text-slate-600 text-sm mt-2">All eyes this way</p>
                    </motion.div>

                    {/* Seats Grid */}
                    <motion.div
                        variants={itemVariants}
                        className="bg-gradient-to-r from-slate-800/50 to-slate-700/50 backdrop-blur-xl border border-slate-200 rounded-2xl p-8 shadow-2xl"
                    >
                                    ...
                                    <div className="flex gap-2 flex-wrap">
                                        {groupedSeats[row].map((seat) => (
                                            <motion.button
                                                ...
                                                className={`w-10 h-10 rounded-lg text-xs font-bold text-white shadow-lg transition-all duration-300 ${seat.isBooked || seat.isLocked
                ...
                >
                    ...
                    >
                        <h3 className="text-xl font-bold text-slate-700 mb-4">Seat Legend</h3>
                        <div className="space-y-3">
                            <div className="flex items-center gap-3">
                                <div className="w-6 h-6 bg-gradient-to-r from-green-400 to-emerald-500 rounded"></div>
                                <span className="text-slate-100">Available</span>
                            </div>
                            <div className="flex items-center gap-3">
                                <div className="w-6 h-6 bg-gradient-to-r from-black to-slate-700 rounded"></div>
                                <span className="text-slate-100">Selected</span>
                            </div>
                            <div className="flex items-center gap-3">
                                <div className="w-6 h-6 bg-red-500 rounded"></div>
                                <span className="text-slate-100">Booked</span>
                            </div>
                            <div className="flex items-center gap-3">
                                <div className="w-6 h-6 bg-yellow-400 rounded"></div>
                                <span className="text-slate-100">Locked</span>
                            </div>
                        </div>
                    </motion.div>
                    ...
                    {selectedSeats.length > 0 && (
                        <motion.div
                            ...
                            className="bg-gradient-to-r from-black/10 to-slate-700/10 backdrop-blur-xl border border-black/30 rounded-2xl p-6 shadow-xl"
                        >
                            <h3 className="text-xl font-bold text-slate-700 mb-4">Your Selection</h3>

                            <div className="space-y-3 mb-4">
                                <div className="flex justify-between items-center">
                                    <span className="text-slate-700">Seats Selected</span>
                                    <span className="text-slate-700 font-semibold">{selectedSeats.length}</span>
                                </div>
                                <div className="flex justify-between items-center">
                                    <span className="text-slate-700">Total Amount</span>
                                    <span className="text-2xl font-bold text-transparent bg-gradient-to-r from-black to-slate-700 bg-clip-text">
                                        ₹{totalAmount}
                                    </span>
                                </div>
                            </div>
            ...
            <motion.footer
                ...
                className="mt-8 bg-gradient-to-r from-black/5 to-slate-200 border-t border-slate-200 rounded-t-2xl p-6 shadow-2xl"
            >
                <div className="max-w-7xl mx-auto flex flex-col sm:flex-row justify-between items-center gap-4">
                    ...
                    {selectedSeats.length > 0 && (
                        <motion.button
                            ...
                            whileTap={{ scale: 0.95 }}
                            className="px-8 py-4 bg-gradient-to-r from-black to-slate-700 text-white font-bold rounded-xl hover:from-slate-700 hover:to-slate-500 transition-all shadow-lg hover:shadow-xl text-lg"
                        >
